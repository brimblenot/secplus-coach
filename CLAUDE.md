...
- `lib/estimates.ts` — Haiku batch per-topic time estimates (cached in `topic_progress.study_minutes`)
...
- **claude-haiku-4-5-20251001** (Haiku 4.5): topic time estimates (`lib/estimates.ts`, batched + cached), in-session chat Q&A
...
1. Student takes main quiz → MC answers collected.
2. Any wrong MC → `POST /api/quiz/second-chance` regenerates questions on the same concepts.
3. Only concepts wrong on **both** passes get flagged → `POST /api/quiz/save` with `weakAreaIndices` (subset of `wrongIndices`).
4. Save route calls Claude to extract specific concept names from wrong questions → `upsertWeakArea` per concept.
5. Pass thresholds: **70%** for a topic quiz, **80%** for a domain mastery quiz.
6. Free-text questions are graded separately via `POST /api/quiz/grade`.
...
### Density Scheduling (`app/api/progress/route.ts`)

On the first load of the day, `getDailyPlan(today)` returns null → a plan is generated by filling topics up to `DAILY_BUDGET_MINUTES` (75 min) using AI-estimated per-topic minutes cached in `topic_progress.study_minutes`. The plan is saved via `saveDailyPlan()` and reused on all later loads that day. `getTopicsCompletedOn(date)` uses SQLite's `date(completed_at)` to find topics finished today, separating planned progress from extra completions.
