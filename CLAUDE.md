...
- **Study guide** (`STUDY_GUIDE_SYSTEM_PROMPT` in `lib/prompts.ts`, consumed by `app/api/session/route.ts`) — rule 1 ("only the transcript may be TAUGHT as exam material; don't introduce untaught topics; Exam-flags must be transcript-covered"). The guide is the only source the topic quiz sees, so a leak here propagates. **Carve-out (rule 2):** the guide MAY add brief one-clause plain-language glosses for any term the lecture names but doesn't define (e.g. what a TPM is) — these are reading aids only, NOT new testable scope. Defining a named term ≠ adding a new topic.
